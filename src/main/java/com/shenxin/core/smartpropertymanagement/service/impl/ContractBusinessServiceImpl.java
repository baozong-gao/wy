...
/**
 * @Author: gaobaozong
 * @Description: 商业楼宇合同服务
 * @Date: Created in 2018/7/23 - 15:45
 * @Version: V1.0
 */
@Service
public class ContractBusinessServiceImpl extends IBaseService<ContractInfoDAO, ContractInfoDAOExample, ContractInfoDAOMapper> {
    ...
    @Autowired
    public ContractBusinessServiceImpl(ContractInfoDAOMapper mapper, IdGenerateService idGenerate) {
    ...
    public ContractInfoDAO searchByRoomId(BigDecimal roomId){
        ...
        ContractRoomInfoDAOExample example = new ContractRoomInfoDAOExample();
        example.createCriteria().andRoomIdEqualTo(roomId).andStatusEqualTo(SystemConstant.STATUS_NORMAL).andCompanyIdEqualTo(UserUtil.getUserCompanyId());
